...
kenh Microsoft Teams khi phat hien mua/sap mua, hoac khi mot khu vuc het mua.
...
- `src/notifier_teams.py` - gui Adaptive Card (co Table nguyen sinh theo tung muc do mua) vao Teams qua Workflows webhook
...
2. Tao webhook Teams: trong group chat/kenh muon nhan canh bao -> `...` -> **Workflows**
   -> chon template gui webhook (vd "Send webhook alerts to a chat") -> o action
   **"Post card in a chat or channel"**, o "Group chat" phai **chon tu danh sach xo xuong**
   (khong go tay ten), neu khong se bi loi "Invalid ThreadId" -> lay URL webhook sinh ra.
3. Vao repo Settings -> Secrets and variables -> Actions, tao 2 secret:
   - `WEATHERAPI_KEY`
   - `TEAMS_WEBHOOK_URL`
4. Push code len repo. Workflow se tu chay moi 5 phut (co the tre do lich GitHub Actions
